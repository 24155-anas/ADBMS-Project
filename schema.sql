...
CREATE TABLE roles (
    ...
    role_name  VARCHAR(20)  NOT NULL UNIQUE
        CHECK (role_name IN ('customer', 'driver', 'admin'))
);
CREATE TABLE users (
...
CREATE TABLE carpool_bookings (
    ...
    created_at    TIMESTAMP   NOT NULL DEFAULT CURRENT_TIMESTAMP,
    UNIQUE (carpool_id, passenger_id)   -- passenger can't book same carpool twice
);
CREATE TABLE payments (
    payment_id      SERIAL         PRIMARY KEY,
    user_id         INTEGER        NOT NULL REFERENCES users(user_id) ON DELETE CASCADE,
    booking_type    VARCHAR(20)    NOT NULL
        CHECK (booking_type IN ('rental', 'ride', 'carpool')),
...
-- Trigger 1-> When a rental booking becomes active mark vehicle unavailable
--            When it becomes completed or cancelled mark vehicle available again
...
-- Trigger 2-> When a carpool booking is inserted decrement available seats
--            in the corresponding carpool offer
--            If seats reach 0 set status to full
...
SELECT
    ...
    COALESCE(ride_totals.ride_earnings, 0)
      + COALESCE(carpool_totals.carpool_earnings, 0) AS total_earnings
FROM users u
LEFT JOIN (
...
LEFT JOIN (
    SELECT co.driver_id,
           SUM(cb.seats_booked * co.price_per_seat) AS carpool_earnings
